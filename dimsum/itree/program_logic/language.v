...
(* From dimsum.core.itree.program_logic Require Export bi_weakestpre. *)
...
Section language_mixin.
  ...
  Context (prim_step : expr → state → list observation → expr → state → list expr → bool → Prop).

  Record LanguageMixin := {
    mixin_to_of_val v : to_val (of_val v) = Some v;
    mixin_of_to_val e v : to_val e = Some v → of_val v = e;
    mixin_val_stuck e σ κ e' σ' efs yld : prim_step e σ κ e' σ' efs yld → to_val e = None
  }.
End language_mixin.
...
Structure language := Language {
  ...
  prim_step : expr → state → list observation → expr → state → list expr → bool → Prop;
...
Global Arguments prim_step {_} _ _ _ _ _ _ _.
...
Definition cfg (Λ : language) := (nat * list (expr Λ) * state Λ)%type.

Class LanguageCtx {Λ : language} (K : expr Λ → expr Λ) := {
  fill_not_val e :
    to_val e = None → to_val (K e) = None;
  fill_step e1 σ1 κ e2 σ2 efs yld :
    prim_step e1 σ1 κ e2 σ2 efs yld →
    prim_step (K e1) σ1 κ (K e2) σ2 efs yld;
  fill_step_inv e1' σ1 κ e2 σ2 efs yld :
    to_val e1' = None → prim_step (K e1') σ1 κ e2 σ2 efs yld →
    ∃ e2', e2 = K e2' ∧ prim_step e1' σ1 κ e2' σ2 efs yld
}.
...
Section language.
  ...
  Proof. apply language_mixin. Qed.
  Lemma val_stuck e σ κ e' σ' efs yld : prim_step e σ κ e' σ' efs yld → to_val e = None.
  Proof. apply language_mixin. Qed.

  Definition reducible (e : expr Λ) (σ : state Λ) :=
    ∃ κ e' σ' efs yld, prim_step e σ κ e' σ' efs yld.
  (* Total WP only permits reductions without observations *)
  Definition reducible_no_obs (e : expr Λ) (σ : state Λ) :=
    ∃ e' σ' efs yld, prim_step e σ [] e' σ' efs yld.
  ...
  Inductive step (ρ1 : cfg Λ) (κ : list (observation Λ)) (ρ2 : cfg Λ) : Prop :=
    | step_atomic i1 i2 e1 σ1 e2 σ2 efs t (yld : bool) :
       ρ1 = (i1, t, σ1) →
       t !! i1 = Some e1 →
       ρ2 = (i2, (<[i1 := e2]> t) ++ efs, σ2) →
       (if yld then True else i1 = i2) →
       prim_step e1 σ1 κ e2 σ2 efs yld →
       step ρ1 κ ρ2.
  ...
  Proof. unfold Atomic. destruct a; naive_solver eauto using val_irreducible. Qed.
  ...
  Proof.
    intros ? (e'&σ'&k&efs&yld&Hstep); unfold reducible.
    apply fill_step_inv in Hstep as (e2' & _ & Hstep); eauto 8.
  Qed.
  ...
  Proof.
    intros ? (e'&σ'&efs&yld&Hstep); unfold reducible_no_obs.
...
*)
  Record pure_step (e1 e2 : expr Λ) := {
    ...
    pure_step_det σ1 κ e2' σ2 efs yld :
      prim_step e1 σ1 κ e2' σ2 efs yld → κ = [] ∧ σ2 = σ1 ∧ e2' = e2 ∧ efs = [] ∧ yld = true
  }.
  ...
  Lemma prim_step_not_stuck e σ κ e' σ' efs yld :
    prim_step e σ κ e' σ' efs yld → not_stuck e σ.
