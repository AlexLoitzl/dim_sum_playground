...
(** * rec_heap_bij *)
(** [rec_heap_bij] allows bijections of memory when proving a
...
Global Instance heap_bijUR_discrete : CmraDiscrete heap_bijUR.
Proof. apply _. Qed.

Program Definition own_heap_s : BiOwn (uPredI heap_bijUR) heapUR := {|
...
Program Definition own_heap_i : BiOwn (uPredI heap_bijUR) heapUR := {|
...
Definition heap_bijUR_shared_inj (r : (gmap_viewUR prov (agreeR locO))) : heap_bijUR
  := (None, (r, ε)).


(** * ghost theory *)
(** ** Ghost state definitions *)
Definition heap_bij_shared_auth_raw (ps : gmap prov loc) : uPred (heap_bijUR) :=
  uPred_ownM (heap_bijUR_shared_inj $ gmap_view_auth (DfracOwn 1) (to_agree <$> ps)).
Definition heap_bij_shared (ps : prov) (li : loc) : uPred (heap_bijUR) :=
  uPred_ownM (heap_bijUR_shared_inj $ gmap_view_frag ps DfracDiscarded (to_agree li)).
Definition heap_bij_statics (provs : gset prov) :=
  uPred_ownM (heap_bijUR_statics_inj (to_agree provs)).

Definition heap_bij_shared_auth (ps : gmap prov loc) : uPred (heap_bijUR) :=
  heap_bij_shared_auth_raw ps ∗ [∗ map] ps↦li0∈ps, heap_bij_shared ps li0.

Notation heap_bij_inv_s := (heapUR_inv own_heap_s).
Notation heap_bij_inv_i := (heapUR_inv own_heap_i).
...
Lemma heap_bij_shared_alloc_raw ps li0 bij :
  bij !! ps = None →
  heap_bij_shared_auth_raw bij ⊢ |==>
  heap_bij_shared_auth_raw (<[ps := li0]>bij) ∗ heap_bij_shared ps li0.
Proof.
...
Lemma heap_bij_shared_alloc ps li0 bij :
  bij !! ps = None →
  heap_bij_shared_auth bij ⊢ |==>
  heap_bij_shared_auth (<[ps := li0]>bij) ∗ heap_bij_shared ps li0.
Proof.
  iIntros (?) "[??]". iMod (heap_bij_shared_alloc_raw with "[$]") as "[$ #$]"; [done|].
  iModIntro. by iApply big_sepM_insert_2.
Qed.

Lemma heap_bij_shared_lookup_raw ps li0 bij  :
  heap_bij_shared_auth_raw bij -∗
  heap_bij_shared ps li0 -∗
  ⌜bij !! ps = Some li0⌝.
Proof.
...
Lemma heap_bij_shared_lookup ps li0 bij  :
  heap_bij_shared_auth bij -∗
  heap_bij_shared ps li0 -∗
  ⌜bij !! ps = Some li0⌝.
Proof. iIntros "[? _]". by iApply heap_bij_shared_lookup_raw. Qed.

Lemma heap_bij_shared_lookup_big bij' bij  :
  heap_bij_shared_auth bij -∗
  ([∗map] ps↦li0∈bij', heap_bij_shared ps li0) -∗
  ⌜bij' ⊆ bij⌝.
Proof.
  iIntros "Ha Hl". iInduction bij' as [|] "IH" using map_ind.
...
Lemma heap_bij_shared_alloc_big_raw bij bij' :
  bij ⊆ bij' →
  heap_bij_shared_auth bij ⊢ |==>
  heap_bij_shared_auth bij'.
Proof.
  iIntros (Hsub) "?". rewrite -(map_difference_union bij bij') //.
  have Hdisj : (bij ##ₘ bij' ∖ bij) by apply map_disjoint_difference_r'.
  rewrite map_union_comm //.
  iInduction (bij' ∖ bij) as [|] "IH" using map_ind forall (Hdisj).
  { by rewrite left_id_L. }
  move: Hdisj => /map_disjoint_insert_r[??].
  iMod ("IH" with "[%] [$]") as "?"; [done|].
  rewrite -insert_union_l.
  iMod (heap_bij_shared_alloc with "[$]") as "[$ _]". 2: done.
  by apply lookup_union_None.
Qed.

Lemma heap_bij_shared_alloc_big bij bij' :
  bij ⊆ bij' →
  heap_bij_shared_auth bij ⊢ |==>
  heap_bij_shared_auth bij' ∗ [∗ map] ps↦li0∈bij', heap_bij_shared ps li0.
Proof.
  iIntros (Hsub) "?".
  iMod (heap_bij_shared_alloc_big_raw with "[$]") as "[? ?]"; [done|].
  iModIntro. iSplit!. iFrame.
Qed.

Lemma heap_bij_shared_auth_shared bij :
  heap_bij_shared_auth bij ⊢ [∗ map] ps↦li0∈bij, heap_bij_shared ps li0.
Proof. iIntros "[? $]". Qed.

Global Typeclasses Opaque heap_bij_shared_auth.

(** * val_in_bij *)
Definition loc_in_bij (li ls : loc) : uPred heap_bijUR :=
  ∃ li0, ⌜li = li0 +ₗ ls.2⌝ ∗ heap_bij_shared ls.1 li0.

Global Instance loc_in_bij_Persistent li ls : Persistent (loc_in_bij li ls).
Proof. apply _. Qed.

Definition val_in_bij (vi vs : val) : uPred heap_bijUR :=
  match vi, vs with
...
Global Instance val_in_bij_Persistent vi vs : Persistent (val_in_bij vi vs).
...
(** * heap_in_bij *)
(* TODO: make this an injection. See
 ...
 *)
Definition heap_in_bij_inv (bij : gmap prov loc) (rem : list prov) :
  uPred heap_bijUR :=
    [∗ map] ps↦li0∈bij, ⌜ps ∈ rem⌝ ∨
      ∃ bi bs, ⌜li0.2 = 0%Z⌝ ∗ li0.1 ↦∗hi bi ∗ ps ↦∗hs bs ∗
      [∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs.

Definition heap_in_bij (rem : list prov) : uPred heap_bijUR :=
  ∃ bij, heap_bij_shared_auth bij ∗ heap_in_bij_inv bij rem.

Lemma heap_in_bij_inv_borrow ps li0 rem bij :
  ps ∉ rem →
  bij !! ps = Some li0 →
  heap_in_bij_inv bij rem -∗
  ∃ bi bs, ⌜li0.2 = 0%Z⌝ ∗ heap_in_bij_inv bij (ps::rem) ∗
  li0.1 ↦∗hi bi ∗ ps ↦∗hs bs ∗ [∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs.
Proof.
  iIntros (??) "Hbij".
  iDestruct (big_sepM_lookup_acc_impl with "Hbij") as "[[%|[% [% [% $]]]] Hbij]";[done..|].
  iSplit!. iApply "Hbij". 2: by iLeft; iPureIntro; set_solver.
  iIntros "!>" (????) "[%|$]". by iLeft; iPureIntro; set_solver.
Qed.

Lemma heap_in_bij_inv_split bij' bij :
  bij' ⊆ bij →
  heap_in_bij_inv bij [] ⊣⊢
  heap_in_bij_inv bij' [] ∗ heap_in_bij_inv (bij ∖ bij') [].
Proof.
  move => ?. rewrite /heap_in_bij_inv -big_sepM_union ?map_difference_union//.
  apply map_disjoint_difference_r'.
Qed.

Lemma heap_in_bij_inv_combine bij' bij :
  heap_in_bij_inv bij [] -∗
  heap_in_bij_inv bij' [] -∗
  heap_in_bij_inv (bij ∪ bij') [].
Proof. apply: big_sepM_union_2. Qed.

Lemma heap_in_bij_inv_return i ps bi bs li0 rem bij :
  rem !! i = Some ps →
  bij !! ps = Some li0 →
  li0.2 = 0%Z →
  heap_in_bij_inv bij rem -∗
  ...
  ps ↦∗hs bs -∗
  ([∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs) -∗
  heap_in_bij_inv bij (delete i rem).
Proof.
  iIntros (???) "Hbij Hi Hs Hvs".
  iDestruct (big_sepM_lookup_acc_impl with "Hbij") as "[Hprev Hbij]";
    [done|].
  iApply "Hbij".
...
Lemma heap_in_bij_inv_return0 ps bi bs li0 rem bij :
  bij !! ps = Some li0 →
  li0.2 = 0%Z →
  heap_in_bij_inv bij (ps :: rem) -∗
  li0.1 ↦∗hi bi -∗
  ps ↦∗hs bs -∗
  ([∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs) -∗
  heap_in_bij_inv bij rem.
Proof. iIntros (??) "????". by iApply (heap_in_bij_inv_return 0 with "[$] [$] [$]"). Qed.


Lemma heap_in_bij_init :
  heap_bij_shared_auth ∅ -∗ heap_in_bij [].
Proof. iIntros "$". by iApply big_sepM_empty. Qed.

Lemma heap_in_bij_borrow ps li0 rem :
  ps ∉ rem →
  heap_in_bij rem -∗
  heap_bij_shared ps li0 -∗
  ∃ bi bs, ⌜li0.2 = 0%Z⌝ ∗ heap_in_bij (ps :: rem) ∗
  li0.1 ↦∗hi bi ∗ ps ↦∗hs bs ∗ [∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs.
Proof.
  iIntros (?) "[%bij [? Hbij]] Hsh".
  iDestruct (heap_bij_shared_lookup with "[$] [$]") as %?.
  iDestruct (heap_in_bij_inv_borrow with "[$]") as (???) "[$ [$ $]]" => //.
  by iFrame.
Qed.

Lemma heap_in_bij_return i ps bi bs li0 rem :
  rem !! i = Some ps →
  li0.2 = 0%Z →
  heap_in_bij rem -∗
  heap_bij_shared ps li0 -∗
  ...
  ps ↦∗hs bs -∗
  ([∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs) -∗
  heap_in_bij (delete i rem).
Proof.
  iIntros (??) "[%bij [? Hbij]] Hsh Hi Hs Hvs".
  iDestruct (heap_bij_shared_lookup with "[$] [$]") as %?.
  iExists _. iFrame. by iApply (heap_in_bij_inv_return with "[$] [$] [$]").
Qed.

Lemma heap_in_bij_return0 ps bi bs li0 rem :
  li0.2 = 0%Z →
  heap_in_bij (ps :: rem) -∗
  heap_bij_shared ps li0 -∗
  li0.1 ↦∗hi bi -∗
  ...
  ([∗ map] o↦vi;vs∈bi;bs, val_in_bij vi vs) -∗
  heap_in_bij rem.
Proof. iIntros (?) "????". by iApply (heap_in_bij_return 0 with "[$] [$] [$] [$]"). Qed.


Lemma heap_in_bij_range hi hs l1 l2 z rem:
  l2.2 = 0%Z →
  heap_range hs l2 z →
  l2.1 ∉ rem →
  heap_bij_inv_i hi -∗
...
Proof.
  ...
  iDestruct (heap_in_bij_borrow with "[$] [$]") as (bi bs ?) "[?[?[??]]]"; [done|].
...
Lemma heap_in_bij_lookup hi hs l1 l2 v rem:
  h_heap hs !! l2 = Some v →
  l2.1 ∉ rem →
  heap_bij_inv_i hi -∗
  heap_bij_inv_s hs -∗
  heap_in_bij rem -∗
  loc_in_bij l1 l2 -∗
  ∃ v', ⌜h_heap hi !! l1 = Some v'⌝ ∗ val_in_bij v' v.
Proof.
  iIntros (??) "Hinvi ? Hh [% [-> ?]]".
  iDestruct (heap_in_bij_borrow with "[$] [$]") as (bi bs Hl0) "[?[?[??]]]"; [done|].
...
Lemma heap_in_bij_update h1 h2 l1 l2 v1 v2 rem:
  l2.1 ∉ rem →
  heap_alive h2 l2 →
  heap_bij_inv_i h1 -∗
  heap_bij_inv_s h2 -∗
  ...
  loc_in_bij l1 l2 -∗
  val_in_bij v1 v2 ==∗
  heap_bij_inv_i (heap_update h1 l1 v1) ∗
  heap_bij_inv_s (heap_update h2 l2 v2) ∗
  heap_in_bij rem.
Proof.
  iIntros (? [? Ha]) "Hinvi Hinvs Hbij [% [-> #?]] Hv".
  iDestruct (heap_in_bij_borrow with "[$] [$]") as (bi bs Hli0) "[?[Hli[??]]]"; [done|].
  ...
  iApply (heap_in_bij_return0 with "[$] [$] [$] [$]"); [done|].
  by iApply (big_sepM2_insert_2 with "[Hv] [$]").
Qed.

Lemma heap_in_bij_share li ls rem bi bs :
  ls.1 ∉ rem →
  li.2 = 0%Z →
  ls.2 = 0%Z →
  heap_in_bij rem -∗
...
Proof.
  iIntros (? ? Hl0) "[%bij[??]] ???".
  destruct (bij !! ls.1) eqn:?. {
    iDestruct (heap_in_bij_inv_borrow with "[$]") as (???) "[? [?[??]]]"; [done..|].
    iDestruct (heapUR_block_excl with "[$] [$]") as %[]. }
  iMod (heap_bij_shared_alloc with "[$]") as "[? #$]"; [done|]. iModIntro.
  iSplit. 2: { iPureIntro. by rewrite Hl0 offset_loc_0. } iFrame.
  iApply big_sepM_insert; [done|]. iFrame. iRight. by iFrame.
Qed.

Lemma heap_in_bij_free hi hs li ls rem:
  ls.1 ∉ rem →
  ls.2 = 0%Z →
  heap_bij_inv_i hi -∗
  heap_bij_inv_s hs -∗
  ...
  heap_in_bij rem ==∗
  heap_in_bij rem ∗
  heap_bij_inv_i (heap_free hi li) ∗
  heap_bij_inv_s (heap_free hs ls).
Proof.
  iIntros (? Hls2) "Hinvi Hinvs [% [-> #Hl]] Hbij".
  rewrite Hls2 offset_loc_0.
  iDestruct (heap_in_bij_borrow with "[$] [$]") as (???) "[? [? [??]]]"; [done|].
  iDestruct (heapUR_lookup_block with "Hinvi [$]") as %<-.
  iDestruct (heapUR_lookup_block with "Hinvs [$]") as %<-.
...
(** * heap_bij_inv *)
Definition heap_bij_inv (hi hs : heap_state) (rem : list prov) : uPred heap_bijUR :=
  heap_bij_inv_i hi ∗
  heap_bij_inv_s hs ∗
  heap_in_bij rem ∗
...
Lemma heap_bij_inv_range hi hs l1 l2 z rem:
  l2.2 = 0%Z →
  heap_range hs l2 z →
  l2.1 ∉ rem →
  heap_bij_inv hi hs rem -∗
  loc_in_bij l1 l2 -∗
  ⌜heap_range hi l1 z⌝.
Proof.
  iIntros (???) "[Hinvi [Hinvs[?[??]]]]".
  by iApply (heap_in_bij_range with "[$] [$] [$]").
Qed.

Lemma heap_bij_inv_lookup hi hs li ls v rem:
  h_heap hs !! ls = Some v →
  ls.1 ∉ rem →
  heap_bij_inv hi hs rem -∗
  loc_in_bij li ls -∗
...
Lemma heap_bij_inv_alive hi hs li ls rem:
  heap_alive hs ls →
  ls.1 ∉ rem →
  heap_bij_inv hi hs rem -∗
  loc_in_bij li ls -∗
...
Lemma heap_bij_inv_lookup_dom_i p hi hs d rem dq:
  heap_bij_inv hi hs rem -∗
  p ⤚hi{dq} d -∗
  ⌜dom (h_block hi p) = d⌝.
Proof.
...
Lemma heap_bij_inv_lookup_dom_s p hi hs d rem dq:
  heap_bij_inv hi hs rem -∗
  p ⤚hs{dq} d -∗
  ⌜dom (h_block hs p) = d⌝.
Proof.
...
Lemma heap_bij_inv_lookup_block_i p hi hs b rem dq:
  heap_bij_inv hi hs rem -∗
  p ↦∗hi{dq} b -∗
  ⌜h_block hi p = b⌝.
Proof.
...
Lemma heap_bij_inv_lookup_block_s p hi hs b rem dq:
  heap_bij_inv hi hs rem -∗
  p ↦∗hs{dq} b -∗
  ⌜h_block hs p = b⌝.
Proof.
...
Lemma heap_bij_inv_update hi hs li ls vi vs rem :
  ls.1 ∉ rem →
  heap_alive hs ls →
  heap_bij_inv hi hs rem -∗
  loc_in_bij li ls -∗
  val_in_bij vi vs ==∗
  heap_bij_inv (heap_update hi li vi) (heap_update hs ls vs) rem.
Proof.
  iIntros (??) "(Hinvi&Hinvs&?&?&?) ? ?".
  iMod (heap_in_bij_update with "[$] [$] [$] [$] [$]") as "[$ [$ $]]"; [done..|].
  iModIntro. rewrite !h_static_provs_heap_update. iFrame.
Qed.

Lemma heap_bij_inv_update_in_block_i hi hs li v rem b :
  heap_alive hi li →
  heap_bij_inv hi hs rem -∗
  li.1 ↦∗hi b ==∗
  heap_bij_inv (heap_update hi li v) hs rem ∗
  li.1 ↦∗hi (<[li.2 :=v]> b).
Proof.
  iIntros (?) "(?&$&$&?&$) ?".
...
Lemma heap_bij_inv_update_in_block_s hi hs ls vs rem b :
  heap_alive hs ls →
  heap_bij_inv hi hs rem -∗
  ls.1 ↦∗hs b ==∗
  heap_bij_inv hi (heap_update hs ls vs) rem ∗
  ls.1 ↦∗hs (<[ls.2:=vs]> b).
Proof.
  iIntros (?) "($&Hinvs&$&$&?) ?".
  iMod (heapUR_update_in_block with "[$] [$]") as "[$ $]"; [done|].
...
Lemma heap_bij_inv_alloc_i hi hs li n rem:
  heap_is_fresh hi li →
  heap_bij_inv hi hs rem ==∗
  heap_bij_inv (heap_alloc hi li n) hs rem ∗
...
Lemma heap_bij_inv_alloc_s hi hs ls n rem:
  heap_is_fresh hs ls →
  heap_bij_inv hi hs rem ==∗
  heap_bij_inv hi (heap_alloc hs ls n) rem ∗
...
Lemma heap_bij_inv_alloc hi hs li ls n rem:
  heap_is_fresh hi li →
  heap_is_fresh hs ls →
  ls.1 ∉ rem →
  heap_bij_inv hi hs rem ==∗
  heap_bij_inv (heap_alloc hi li n) (heap_alloc hs ls n) rem ∗
  loc_in_bij li ls.
Proof.
  iIntros (Hi Hs ?) "?". have Hli0 : li.2 = 0 by destruct Hi as [?[??]].
  iMod (heap_bij_inv_alloc_i with "[$]") as "[? ?]"; [done|].
  iMod (heap_bij_inv_alloc_s with "[$]") as "[Hinv ?]"; [done|].
  iDestruct "Hinv" as "($ & $ &?& $ & $)".
  iMod (heap_in_bij_share with "[$] [$] [$] []") as "[$ $]"; [done..| | |done].
  { destruct Hs. naive_solver. }
  iApply big_sepM_sepM2_diag. iApply big_sepM_intro.
  iIntros "!>" (??[-> ?]%zero_block_lookup_Some). done.
Qed.

Lemma heap_bij_inv_alloc_list hi hs hi' hs' xs lsi lss:
  heap_alloc_list xs lsi hi hi' →
  heap_alloc_list xs lss hs hs' →
  heap_bij_inv hi hs [] ==∗
  heap_bij_inv hi' hs' [] ∗
  ([∗ list] li;ls∈lsi;lss, loc_in_bij li ls).
Proof.
  iIntros (Hi Hs) "Hinv".
  iInduction xs as [] "IH" forall (lsi lss hi hi' hs hs' Hi Hs); simplify_eq/=; destruct!/=.
  { by iFrame. }
  iMod (heap_bij_inv_alloc with "Hinv") as "[Hinv $]"; [done..|set_solver|].
  by iApply "IH".
Qed.

Lemma heap_bij_inv_free_i l hi hs b rem:
  heap_bij_inv hi hs rem -∗
  l.1 ↦∗hi b ==∗
  heap_bij_inv (heap_free hi l) hs rem ∗ l.1 ↦∗hi ∅.
Proof.
  iIntros "[Hinvi [Hinvs[?[??]]]] ?".
...
Lemma heap_bij_inv_free_s l hi hs b rem:
  heap_bij_inv hi hs rem -∗
  l.1 ↦∗hs b ==∗
  heap_bij_inv hi (heap_free hs l) rem ∗ l.1 ↦∗hs ∅.
Proof.
  iIntros "[Hinvi [Hinvs[?[??]]]] ?".
...
Lemma heap_bij_inv_free hi hs li ls rem n:
  ls.1 ∉ rem →
  li.2 = 0%Z →
  ls.2 = 0%Z →
  heap_range hs ls n →
  heap_bij_inv hi hs rem -∗
  loc_in_bij li ls ==∗
  heap_bij_inv (heap_free hi li) (heap_free hs ls) rem.
Proof.
  iIntros (? ?? ?) "[Hinvi [Hinvs [? [? ?]]]] Hl".
  iMod (heap_in_bij_free with "[$] [$] [$] [$]") as "[$ [$ $]]"; [done..|].
  rewrite !h_static_provs_heap_free. by iFrame.
Qed.

Lemma heap_bij_inv_free_list hi hs hs' lis lss lis' lss' xsi xss hi0 hi0' hs0 hs0':
...
Proof.
  ...
  iAssert ⌜heap_range hi l0 z⌝%I as %?. {
    iDestruct "Hinv" as "(?&?&?&?)".
    iApply (heap_in_bij_range with "[$] [$] [$] [$]"); [done..| set_solver].
  }
  iMod (heap_bij_inv_free with "[$] [$]") as "[??]"; [set_solver|done..|].
...
Proof.
  iIntros "($&$&?&$&$) Hi Hs #Hv".
  iMod (heap_in_bij_share (p, 0%Z) (p, 0%Z) with "[$] [$] [$] []") as "[$ $]"; [set_solver|done|done| |done].
...
Lemma heap_bij_init ps :
  satisfiable (heap_bij_shared_auth ∅ ∗ heap_bij_statics ps
                 ∗ heap_bij_inv_i ∅ ∗ heap_bij_inv_s ∅).
Proof.
  ...
  iIntros!. rewrite -!heapUR_init_own /heap_bij_shared_auth big_sepM_empty. by iFrame.
Qed.
...
Proof.
  ...
  - simpl. simpl in Hstatic.
    ...
    iDestruct (heap_bij_inv_lookup with "Hinv Hv") as "[%w [% #Hw]]"; [done|set_solver|].
    iSatStop. tstep_i. split!. eapply Hcont; first by etrans.
  ...
  - simpl. simpl in Hstatic. eapply andb_True in Hstatic as [Hstatic1 Hstat2].
    ...
    iDestruct (heap_bij_inv_alive with "Hinv Hv") as "%"; [done|set_solver|].
    iMod (heap_bij_inv_update with "Hinv Hv Hw") as "Hinv"; [set_solver|done|]. iModIntro.
    iSatStop. tstep_i. split!. eapply Hcont; first by etrans.
    iSatMono. iFrame. iFrame "Hw".
...
Proof.
  ...
  - iSatMonoBupd. iIntros "(? & Hvs & #HINV & r & $)".
    iMod (heap_bij_inv_alloc_list with "[$]") as "[$ Hl]"; [done..|iDestruct "Hl" as "#Hl"].
    iModIntro.
  ...
  - iSatClear. move => /= ?????????.
    ...
    iMod (heap_bij_inv_free_list with "[$] [$]") as (??) "?"; [done|..]; last (iModIntro; iSatStop; split!; [done|]).
...
Proof.
  ...
  { iSatMonoBupd. iIntros!. iFrame.
    iMod (heap_bij_inv_alloc_s _ _ l with "[$]") as "[? ?]"; [done|].
    iMod (heap_bij_inv_update_in_block_s with "[$] [$]") as "[$ ?]"; [done|].
    iModIntro. iAccu. }
  iSatClear.
  move => v1'' v2'' h1'' h2'' rf'' ? /=.
  iSatStart. simplify_eq/=. iIntros!.
  iDestruct (heap_bij_inv_lookup_block_s with "[$] [$]") as %Hl.
  iSatStop.
  ...
  apply Hret; [done|].
  iSatMonoBupd.
  iMod (heap_bij_inv_free_s with "[$] [$]") as "[$ ?]".
  iModIntro. by iFrame.
Qed.
