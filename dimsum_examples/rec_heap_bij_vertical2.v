...
(** ** Definitions for combining and splitting injections / bijections *)
(** abbreviation: rhbv: rec_heap_bij_vertical *)
(** inj: souce -> impl, injb: source -> middle, inja: middle -> impl *)

(** [rhbv_combine inja injb] combines the small injections [inja] and
[injb] into a large injection *)
Definition rhbv_combine (inja injb : gmap prov loc) : gmap prov loc :=
  omap (λ l, (λ li, li +ₗ l.2) <$> inja !! l.1) injb.

(** [rhbv_core_b inja injb] computes the part of [injb] that is also
used by the large injection *)
Definition rhbv_core_b (inja injb : gmap prov loc) : gmap prov loc :=
  filter (λ x, is_Some (inja !! x.2.1)) injb.

(** [rhbv_core_b inja injb] computes the part of [inja] that is also
used by the large injection *)
Definition rhbv_core_a (inja injb : gmap prov loc) : gmap prov loc :=
  filter (λ x, x.1 ∈ set_map (D:=gset _) fst (map_img (SA:=gset loc) injb)) inja.

(** [rhbv_proj_b inj inja injb X] projects a new b injection [injb'] injection
from the large injection [inj]. It preserves the previous [injb] and
allocates new provenances in the middle from outside [X]. *)
Definition rhbv_proj_b (inj inja injb : gmap prov loc) (X : gset prov) : gmap prov loc :=
  injb ∪ ((λ p, (p, 0%Z)) <$> fresh_map (dom inj) X).

(** [rhbv_proj_a inj inja injb X] projects a new a injection [inja'] injection
from the large injection [inj]. It preserves the previous [inja] and
allocates new provenances in the middle from outside [X]. *)
Definition rhbv_proj_a (inj inja injb : gmap prov loc) (X : gset prov) : gmap prov loc :=
  inja ∪ kmap (λ p, fresh_map (dom inj) X !!! p) (inj ∖ injb).

(** [rhbv_residual] is what is left in the middle heap after the
small injections are converted to large injection. *)
Definition rhbv_residual {Σ} `{!satG Σ heap_bijUR} (γa γb : sat_name _) (l : loc) : iProp Σ :=
  ∃ b, ⌜l.2 = 0⌝ ∗ ⌈l.1 ↦∗hi b @ sat γb⌉ ∗ ⌈l.1 ↦∗hs b @ sat γa⌉.

(** *** Lookup lemmas *)
Lemma rhbv_combine_lookup_Some inja injb ps li0:
  rhbv_combine inja injb !! ps = Some li0 ↔
   ∃ l li', injb !! ps = Some l ∧ inja !! l.1 = Some li' ∧ li0 = li' +ₗ l.2.
Proof.
  rewrite /rhbv_combine lookup_omap_Some. f_equiv => ? /=.
  rewrite fmap_Some. naive_solver.
Qed.

Lemma rhbv_core_b_lookup_Some inja injb p l :
  rhbv_core_b inja injb !! p = Some l ↔
   injb !! p = Some l ∧ is_Some (inja !! l.1).
Proof. by rewrite /rhbv_core_b map_lookup_filter_Some. Qed.

Lemma rhbv_core_b_lookup_None inja injb p :
  rhbv_core_b inja injb !! p = None ↔
   (∀ l, injb !! p = Some l → inja !! l.1 = None).
Proof.
  rewrite eq_None_not_Some /is_Some.
  setoid_rewrite rhbv_core_b_lookup_Some. split.
  2: rewrite /is_Some; naive_solver.
  move => Hl l ?. apply eq_None_not_Some. naive_solver.
Qed.

Lemma rhbv_core_a_lookup_Some inja injb p l :
  rhbv_core_a inja injb !! p = Some l ↔
   inja !! p = Some l ∧ ∃ ps lm, injb !! ps = Some lm ∧ lm.1 = p.
Proof.
  rewrite /rhbv_core_a map_lookup_filter_Some /=.
  rewrite elem_of_map. setoid_rewrite elem_of_map_img.
  naive_solver.
Qed.

Lemma rhbv_proj_b_lookup_Some inj inja injb X p (l : loc) :
  rhbv_proj_b inj inja injb X !! p = Some l ↔
   injb !! p = Some l ∨ injb !! p = None ∧
     fresh_map (dom inj) X !! p = Some l.1 ∧ l.2 = 0.
Proof.
  rewrite /rhbv_proj_b lookup_union_Some_raw. do 2 f_equiv.
  rewrite lookup_fmap_Some. destruct l; naive_solver.
Qed.

Lemma rhbv_proj_a_lookup_Some inj inja injb X p (l : loc) :
  rhbv_proj_a inj inja injb X !! p = Some l ↔
   inja !! p = Some l ∨ inja !! p = None ∧
     ∃ ps, inj !! ps = Some l ∧ injb !! ps = None ∧ fresh_map (dom inj) X !! ps = Some p.
Proof.
  rewrite /rhbv_proj_a lookup_union_Some_raw. do 2 f_equiv.
...
(** [rhbv_combine] is monotonic. *)
Lemma rhbv_combine_subseteq inja injb inja' injb':
  inja ⊆ inja' →
  injb ⊆ injb' →
  rhbv_combine inja injb ⊆ rhbv_combine inja' injb'.
Proof.
  move => Hsuba Hsubb. apply map_subseteq_spec => ??.
  rewrite !rhbv_combine_lookup_Some.
  move => [? [? [/(lookup_weaken _ _ _ _)/(_ Hsubb)? [/(lookup_weaken _ _ _ _)/(_ Hsuba) ??]]]].
  naive_solver.
Qed.

(** [rhbv_core_a] is smaller than [inja] *)
Lemma rhbv_core_a_subseteq inja injb :
  rhbv_core_a inja injb ⊆ inja.
Proof. apply map_subseteq_spec => ?? /rhbv_core_a_lookup_Some. naive_solver. Qed.

(** [rhbv_core_b] is smaller than [inja] *)
Lemma rhbv_core_b_subseteq inja injb :
  rhbv_core_b inja injb ⊆ injb.
Proof. apply map_subseteq_spec => ?? /rhbv_core_b_lookup_Some. naive_solver. Qed.

(** [rhbv_proj_a] is bigger than [inja] *)
Lemma rhbv_proj_a_subseteq inj inja injb X :
  inja ⊆ rhbv_proj_a inj inja injb X.
Proof. apply map_union_subseteq_l. Qed.

(** [rhbv_proj_b] is bigger than [injb] *)
Lemma rhbv_proj_b_subseteq inj inja injb X :
  injb ⊆ rhbv_proj_b inj inja injb X.
Proof. apply map_union_subseteq_l. Qed.

(** We can convert the new large injection [inj] as a combination of
the projections. *)
Lemma rhbv_combine_proj inj inja injb X :
  (** inj only grew *)
  rhbv_combine inja injb ⊆ inj →
  ...
  inj ##ₘ injb ∖ rhbv_core_b inja injb →
  (** newly allocated provs must be disjoint from the existing ones in inja and injb *)
  dom inja ⊆ X →
  map_img (fst <$> injb) ⊆ X →
  inj = rhbv_combine (rhbv_proj_a inj inja injb X) (rhbv_proj_b inj inja injb X).
Proof.
  move => Hsub Hdisj HXa HXb.
  apply map_eq => p. apply option_eq => li.
  rewrite rhbv_combine_lookup_Some.
  setoid_rewrite rhbv_proj_a_lookup_Some.
  setoid_rewrite rhbv_proj_b_lookup_Some.
  split => ?.
  - exploit @map_disjoint_Some_l; [done..|].
    move => /lookup_difference_None[?|[?/rhbv_core_b_lookup_Some[?[??]]]].
    ...
    + move: Hsub =>/map_subseteq_spec Hsub.
      exploit Hsub. { apply rhbv_combine_lookup_Some. naive_solver. }
      naive_solver.
  - destruct!.
    + move: Hsub =>/map_subseteq_spec Hsub. apply Hsub.
      apply rhbv_combine_lookup_Some. naive_solver.
...
Lemma rhbv_core_a_proj_subseteq inja injb inj X :
  rhbv_core_a inja injb ⊆ rhbv_core_a (rhbv_proj_a inj inja injb X) (rhbv_proj_b inj inja injb X).
Proof.
  apply map_subseteq_spec => ?? /rhbv_core_a_lookup_Some [? [? [? [??]]]].
  apply rhbv_core_a_lookup_Some. split.
  - apply rhbv_proj_a_lookup_Some. naive_solver.
  - eexists _, _. split; [|done]. apply rhbv_proj_b_lookup_Some. naive_solver.
Qed.


(** The core of b only grows. *)
Lemma rhbv_core_b_proj_subseteq inja injb inj X :
  rhbv_core_b inja injb ⊆ rhbv_core_b (rhbv_proj_a inj inja injb X) (rhbv_proj_b inj inja injb X).
Proof.
  apply map_subseteq_spec => ?? /rhbv_core_b_lookup_Some [? [??]].
  apply rhbv_core_b_lookup_Some. split.
  - apply rhbv_proj_b_lookup_Some. naive_solver.
  - eexists _. apply rhbv_proj_a_lookup_Some. naive_solver.
Qed.
...
left inclusion.) *)
Lemma rhbv_recombine_a inja injb inja' injb' inj' X :
  inja' = rhbv_proj_a inj' inja injb X →
  injb' = rhbv_proj_b inj' inja injb X →
  (inja ∖ rhbv_core_a inja injb ∪ rhbv_core_a inja' injb') = inja'.
Proof.
  move => -> ->.
  apply map_eq => pm. apply option_eq => li.
  rewrite lookup_union_Some_raw lookup_difference_Some lookup_difference_None.
  split.
  - move => [[??]| [? /rhbv_core_a_lookup_Some [??]]] //.
    apply: lookup_weaken; [done|]. apply rhbv_proj_a_subseteq.
  - move => /rhbv_proj_a_lookup_Some[?|[?[?[?[??]]]]].
    + destruct (rhbv_core_a inja injb !! pm) eqn:Heq; [|naive_solver]. split!.
      apply: lookup_weaken. 2: apply rhbv_core_a_proj_subseteq.
      move: (Heq) => /rhbv_core_a_lookup_Some. naive_solver.
    + split!. by left. apply rhbv_core_a_lookup_Some. split.
      * apply rhbv_proj_a_lookup_Some. naive_solver.
      * eexists _, (_, _). split!. apply rhbv_proj_b_lookup_Some. by right.
Qed.
...
left inclusion.) *)
Lemma rhbv_recombine_b inja injb inja' injb' inj' X :
  inja' = rhbv_proj_a inj' inja injb X →
  injb' = rhbv_proj_b inj' inja injb X →
  dom inja ⊆ X →
  (injb ∖ rhbv_core_b inja injb ∪ rhbv_core_b inja' injb') = injb'.
Proof.
  move => -> -> HXa.
  apply map_eq => ps. apply option_eq => lm.
  rewrite lookup_union_Some_raw lookup_difference_Some lookup_difference_None.
  rewrite rhbv_core_b_lookup_None rhbv_core_b_lookup_Some.
  split.
  - move => [[??]|[?[??]]] //. apply: lookup_weaken; [done|].
    apply rhbv_proj_b_subseteq.
  - move => /rhbv_proj_b_lookup_Some[?|[?[??]]].
    + destruct (inja !! lm.1) eqn:?.
      * right. split!. { eexists _. apply rhbv_core_b_lookup_Some. naive_solver. }
        { apply rhbv_proj_b_lookup_Some. naive_solver. }
        eexists _. apply rhbv_proj_a_lookup_Some. by left.
      * left. split!. move => ??. unfold loc in *. by simplify_eq.
    + split!. by left.
      * apply rhbv_proj_b_lookup_Some. naive_solver.
      * exploit @fresh_map_lookup_Some; [done|].
        move => [/elem_of_dom[??] Hnotin]. eexists _.
        apply rhbv_proj_a_lookup_Some. right. split! => //.
        apply eq_None_not_Some => -[?]. contradict Hnotin. apply HXa.
        by apply elem_of_dom.
Qed.

(** [rhbv_combine_to_b] allows us to express [rhbv_combine] in terms of [rhbv_core_b]. *)
Lemma rhbv_combine_to_b inja injb :
  rhbv_combine inja injb = (λ l, inja !!! l.1 +ₗ l.2) <$> rhbv_core_b inja injb.
Proof.
  apply map_eq => ?. apply option_eq => ?. rewrite rhbv_combine_lookup_Some.
  rewrite lookup_fmap_Some. setoid_rewrite rhbv_core_b_lookup_Some.
...
Lemma rhbv_core_b_proj_fresh inj inja injb X p (l : loc):
  map_img (fst <$> injb) ⊆ X →
  (rhbv_core_b (rhbv_proj_a inj inja injb X) (rhbv_proj_b inj inja injb X) ∖
     rhbv_core_b inja injb) !! p = Some l →
  fresh_map (dom inj) X !! p = Some l.1 ∧ l.2 = 0.
Proof.
  move => HXb /lookup_difference_Some[/rhbv_core_b_lookup_Some[/rhbv_proj_b_lookup_Some Horb [? /rhbv_proj_a_lookup_Some Hora]] /rhbv_core_b_lookup_None?].
...
Section vertical.
Context {Σ} `{!satG Σ heap_bijUR}.

(** Combine the value relation. *)
Lemma rhbv_combine_val inja' injb' γa γb γ vi vm vs:
  ⌈heap_bij_shared_auth inja' @ sat γa⌉ -∗
  ⌈heap_bij_shared_auth injb' @ sat γb⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ rhbv_combine inja' injb', heap_bij_shared ps li0 @ sat γ⌉ -∗
  ⌈val_in_bij vi vm @ sat γa⌉ -∗
  ⌈val_in_bij vm vs @ sat γb⌉ -∗
  ⌈val_in_bij vi vs @ sat γ⌉.
Proof.
  ...
  iDestruct (heap_bij_shared_lookup with "Ha [$]") as %?.
  iDestruct (heap_bij_shared_lookup with "Hb [$]") as %?.
  iDestruct (big_sepM_lookup (PROP:=uPred heap_bijUR) with "[$]") as "?".
  { apply rhbv_combine_lookup_Some. by split!. }
  rewrite /loc_in_bij weak_embed_exist.
  iExists _. iFrame. by rewrite offset_loc_assoc.
Qed.

Lemma rhbv_combine_val_list inja' injb' γa γb γ vi vm vs:
  ⌈heap_bij_shared_auth inja' @ sat γa⌉ -∗
  ⌈heap_bij_shared_auth injb' @ sat γb⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ rhbv_combine inja' injb', heap_bij_shared ps li0 @ sat γ⌉ -∗
  ⌈[∗ list] v1;v2∈vi;vm, val_in_bij v1 v2 @ sat γa⌉ -∗
  ⌈[∗ list] v1;v2∈vm;vs, val_in_bij v1 v2 @ sat γb⌉ -∗
  ⌈[∗ list] v1;v2∈vi;vs, val_in_bij v1 v2 @ sat γ⌉.
Proof.
  iIntros "Haa Hab ? Him Hms".
  iInduction vm as [|] "IH" forall (vi vs). {
    iDestruct (big_sepL2_nil_inv_r (PROP:=uPred heap_bijUR) with "Him") as %->.
  ...
  iDestruct (big_sepL2_cons_inv_r (PROP:=uPred heap_bijUR) with "Him") as (?? ->) "[??]".
  simpl.
  iDestruct (rhbv_combine_val with "Haa Hab [$] [$] [$]") as "#?".
  iSplit; [done|]. iApply ("IH" with "[$] [$] [$] [$] [$]").
Qed.

Lemma rhbv_combine_val_map `{Countable K} inja' injb' γa γb γ (vi vm vs : gmap K val):
  ⌈heap_bij_shared_auth inja' @ sat γa⌉ -∗
  ⌈heap_bij_shared_auth injb' @ sat γb⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ rhbv_combine inja' injb', heap_bij_shared ps li0 @ sat γ⌉ -∗
  ⌈[∗ map] v1;v2∈vi;vm, val_in_bij v1 v2 @ sat γa⌉ -∗
  ⌈[∗ map] v1;v2∈vm;vs, val_in_bij v1 v2 @ sat γb⌉ -∗
  ⌈[∗ map] v1;v2∈vi;vs, val_in_bij v1 v2 @ sat γ⌉.
Proof.
  iIntros "Haa Hab ? Him Hms".
  iInduction vm as [|] "IH" using map_ind forall (vi vs). {
    iDestruct (big_sepM2_empty_l (PROP:=uPred heap_bijUR) with "Him") as %->.
  ...
  { apply lookup_insert. }
  iDestruct (big_sepM2_delete_r (PROP:=uPred heap_bijUR) with "Him") as (??) "[??]".
  { apply lookup_insert. }
...
Lemma rhbv_split_val inja' injb' γa γb γ vi vs:
  ⌈[∗ map] ps↦li0 ∈ inja', heap_bij_shared ps li0 @ sat γa⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ injb', heap_bij_shared ps li0 @ sat γb⌉ -∗
  ⌈heap_bij_shared_auth (rhbv_combine inja' injb') @ sat γ⌉ -∗

  ⌈val_in_bij vi vs @ sat γ⌉ -∗
  ∃ vm, ⌈val_in_bij vi vm @ sat γa⌉ ∗ ⌈val_in_bij vm vs @ sat γb⌉.
Proof.
  iIntros "Ha Hb H Hv".
  ...
  iDestruct (heap_bij_shared_lookup with "H [$]") as %[lm[li'[?[??]]]]%rhbv_combine_lookup_Some.
  iDestruct (big_sepM_lookup (PROP:=uPred heap_bijUR) with "Ha") as "?"; [done|].
  iDestruct (big_sepM_lookup (PROP:=uPred heap_bijUR) with "Hb") as "?"; [done|].
  subst. iExists (ValLoc (lm +ₗ l0.2)) => /=. iFrame. iSplit; [|done].
  unfold loc_in_bij. simpl. iFrame. by rewrite offset_loc_assoc.
Qed.

Lemma rhbv_split_val_list inja' injb' γa γb γ vis vss:
  ⌈[∗ map] ps↦li0 ∈ inja', heap_bij_shared ps li0 @ sat γa⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ injb', heap_bij_shared ps li0 @ sat γb⌉ -∗
  ⌈heap_bij_shared_auth (rhbv_combine inja' injb') @ sat γ⌉ -∗

  ⌈[∗ list] vi;vs∈vis;vss, val_in_bij vi vs @ sat γ⌉ -∗
  ∃ vms, ⌈[∗ list] vi;vm∈vis;vms, val_in_bij vi vm @ sat γa⌉ ∗
         ⌈[∗ list] vm;vs∈vms;vss, val_in_bij vm vs @ sat γb⌉.
Proof.
  iIntros "Ha Hb H Hv".
  iInduction vis as [|] "IH" forall (vss). {
    iDestruct (big_sepL2_nil_inv_l (PROP:=uPred heap_bijUR) with "Hv") as %->.
    iExists []. by simpl.
  }
  iDestruct (big_sepL2_cons_inv_l (PROP:=uPred heap_bijUR) with "Hv") as (?? ->) "[??]".
  iDestruct (rhbv_split_val with "Ha Hb [$] [$]") as (?) "#[??]".
  iDestruct ("IH" with "Ha Hb [$] [$]") as (?) "#[??]".
  iExists (_::_) => /=. by repeat iSplit.
Qed.

Lemma rhbv_split_val_map {A} `{Countable A} inja' injb' γa γb γ (vis vss : gmap A val):
  ⌈[∗ map] ps↦li0 ∈ inja', heap_bij_shared ps li0 @ sat γa⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ injb', heap_bij_shared ps li0 @ sat γb⌉ -∗
  ⌈heap_bij_shared_auth (rhbv_combine inja' injb') @ sat γ⌉ -∗

  ⌈[∗ map] vi;vs∈vis;vss, val_in_bij vi vs @ sat γ⌉ -∗
  ∃ vms, ⌈[∗ map] vi;vm∈vis;vms, val_in_bij vi vm @ sat γa⌉ ∗
         ⌈[∗ map] vm;vs∈vms;vss, val_in_bij vm vs @ sat γb⌉.
Proof.
  iIntros "Ha Hb H Hv".
  iInduction vis as [|] "IH" using map_ind forall (vss). {
    iDestruct (big_sepM2_empty_r (PROP:=uPred heap_bijUR) with "Hv") as %->.
    iExists ∅. by rewrite big_sepM2_empty.
  }
  iDestruct (big_sepM2_delete_l (PROP:=uPred heap_bijUR) with "Hv") as (??) "[??]".
  { apply lookup_insert. }
  rewrite delete_insert //.
  iDestruct (rhbv_split_val with "Ha Hb [$] [$]") as (?) "#[? ?]".
  ...
  all: by iApply (big_sepM2_insert_2 (PROP:=uPred heap_bijUR)); [done|].
Qed.

(** Allocate the residual for the new provs in the middle heap. *)
Lemma rhbv_alloc_residual (m : gmap prov loc) hm γa γb:
  ...
  ⌈{sat γa}⌉ -∗ ⌈{sat γb}⌉ -∗
  ⌈heap_bij_inv_s hm @ sat γa⌉ -∗
  ⌈heap_bij_inv_i hm @ sat γb⌉ ==∗
  ∃ hm', ([∗ map] li0 ∈ m, rhbv_residual γa γb li0) ∗
  ⌈{sat γa}⌉ ∗ ⌈{sat γb}⌉ ∗
  ⌈heap_bij_inv_s hm' @ sat γa⌉ ∗
  ⌈heap_bij_inv_i hm' @ sat γb⌉ ∗
...
[rhbv_core_a] into the [big_sepM] for [rhbv_core_b]. This is useful
since then we only need to do an induction over [rhbv_core_b].
However, this only holds if [rhbv_core_b] is an injection, i.e. it
...
for multiple elements of [rhbv_core_b]./) *)
Lemma big_sepM_rhbv_core_a_b_1 {PROP : bi} inja injb (Φa Φb : _ → _ → PROP) `{!BiAffine PROP} `{!BiPureForall PROP}:
  (** Check that rhbv_core_b is injective *)
  (∀ p1 p2 l1 l2, ⌜l1.1 = l2.1⌝ -∗ Φb p1 l1 -∗ Φb p2 l2 -∗ False) -∗
  ([∗ map] p↦l ∈ rhbv_core_a inja injb, Φa p l) -∗
  ([∗ map] p↦l ∈ rhbv_core_b inja injb, Φb p l) -∗
  ([∗ map] p↦l ∈ rhbv_core_b inja injb,
    Φb p l ∗ ∃ li, ⌜rhbv_core_a inja injb !! l.1 = Some li⌝ ∗ Φa l.1 li).
Proof.
  iIntros "Hf ? Hb".
  iAssert ⌜∀ p1 p2 l1 l2, l1.1 = l2.1 → rhbv_core_b inja injb !! p1 = Some l1 → rhbv_core_b inja injb !! p2 = Some l2 → p1 = p2⌝%I as %Himpl. {
    iIntros (p1 p2 l1 l2 ? ??). destruct (decide (p1 = p2)) => //.
    rewrite (big_sepM_delete _ (rhbv_core_b _ _)); [|done]. iDestruct "Hb" as "[? Hb]".
  ...
  move => ?? /rhbv_core_b_lookup_Some[?[??]].
  split!.
  - apply rhbv_core_a_lookup_Some. naive_solver.
  - iIntros "$". iPureIntro. apply rhbv_core_a_lookup_Some. naive_solver.
  - move => ????. apply: Himpl; [done| |done]. apply rhbv_core_b_lookup_Some. naive_solver.
Qed.

Lemma big_sepM_rhbv_core_a_b_2 {PROP : bi} inja injb (Φa Φb : _ → _ → PROP) `{!BiAffine PROP}:
  ([∗ map] p↦l ∈ rhbv_core_b inja injb,
    Φb p l ∗ ∃ li, ⌜rhbv_core_a inja injb !! l.1 = Some li⌝ ∗ Φa l.1 li) ⊢
  ([∗ map] p↦l ∈ rhbv_core_a inja injb, Φa p l) ∗
  ([∗ map] p↦l ∈ rhbv_core_b inja injb, Φb p l).
Proof.
  ...
  split!.
  - done.
  - apply rhbv_core_b_lookup_Some. naive_solver.
  - iIntros "(%& %Ha & ?)". move: Ha => /rhbv_core_a_lookup_Some[?[?[?[??]]]]. by simplify_eq.
  - naive_solver.
Qed.

Lemma big_sepM_rhbv_core_a_b {PROP : bi} inja injb (Φa Φb : _ → _ → PROP) `{!BiAffine PROP} `{!BiPureForall PROP}:
  (∀ p1 p2 l1 l2, l1.1 = l2.1 → Φb p1 l1 -∗ Φb p2 l2 -∗ False) →
  ([∗ map] p↦l ∈ rhbv_core_a inja injb, Φa p l) ∗
  ([∗ map] p↦l ∈ rhbv_core_b inja injb, Φb p l) ⊣⊢
  ([∗ map] p↦l ∈ rhbv_core_b inja injb,
...
Proof.
  move => Hf. iSplit.
  - iIntros "[??]". iApply (big_sepM_rhbv_core_a_b_1 with "[] [$] [$]"). iIntros. by iApply (Hf with "[$] [$]").
  - iIntros "?". by iApply big_sepM_rhbv_core_a_b_2.
Qed.

(** Create the large invariant and the residual from the two small invariants. *)
Lemma rhbv_shared_from_ab inja' injb' γa γb γ ha hb hm:
  heapUR_trader (sat γ) (sat γa) own_heap_i own_heap_i -∗
  heapUR_trader (sat γ) (sat γb) own_heap_s own_heap_s -∗
  ⌈heap_bij_inv_i ha @ sat γa⌉ -∗
  ⌈heap_bij_inv_s hm @ sat γa⌉ -∗
  ⌈heap_bij_inv_i hm @ sat γb⌉ -∗
  ...
  ⌈heap_in_bij_inv (rhbv_core_a inja' injb') [] @ sat γa⌉ -∗
  ⌈heap_in_bij_inv (rhbv_core_b inja' injb') [] @ sat γb⌉ ==∗⌈sat γ⌉
  (⌈heap_in_bij_inv (rhbv_combine inja' injb') [] @ sat γ⌉ ∗
  ([∗ map] ps↦li0 ∈ rhbv_core_b inja' injb', rhbv_residual γa γb li0)) ∗
  ...
  ⌈heap_bij_shared_auth injb' @ sat γb⌉.
Proof.
  iIntros "???????? #? ? ?". rewrite /heap_in_bij_inv.
  have Hempty : ∀ k, k ∈ [] ↔ False by set_solver.
  setoid_rewrite Hempty. setoid_rewrite bi.False_or.
  iEval (rewrite rhbv_combine_to_b big_sepM_fmap).
  rewrite (weak_embed_big_sepM _ _ (rhbv_core_a _ _)).
  rewrite !(weak_embed_big_sepM _ _ (rhbv_core_b _ _)).
  iDestruct (big_sepM_rhbv_core_a_b_1 with "[] [$] [$]") as "?".
  ...
  iDestruct "Ha" as (?[??]%rhbv_core_a_lookup_Some) "(% & % & % & ? & ? & Hva)" => //=.
  iMod (heapUR_trade_block with "Hta [$] [$]") as "[? [??]]".
  iMod (heapUR_trade_block with "Htb [$] [$]") as "[? [??]]".
  iDestruct (heapUR_lookup_block (PROP:=uPred heap_bijUR) with "Hma [$]") as %?.
  iDestruct (heapUR_lookup_block (PROP:=uPred heap_bijUR) with "Hmb [$]") as %?.
  simplify_eq.
  iDestruct (rhbv_combine_val_map with "Hsha Hshb [$] Hva Hvb") as "#?". iFrame.
...
Lemma rhbv_shared_to_ab inja' injb' γa γb γ ha hb hm:
  ⌈{sat γa}⌉ -∗ ⌈{sat γb}⌉ -∗
  heapUR_trader (sat γa) (sat γ) own_heap_i own_heap_i -∗
  heapUR_trader (sat γb) (sat γ) own_heap_s own_heap_s -∗
  ⌈heap_bij_inv_i ha @ sat γ⌉ -∗
  ⌈heap_bij_inv_s hm @ sat γa⌉ -∗
  ⌈heap_bij_inv_i hm @ sat γb⌉ -∗
  ⌈heap_bij_inv_s hb @ sat γ⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ inja', heap_bij_shared ps li0 @ sat γa⌉ -∗
  ⌈[∗ map] ps↦li0 ∈ injb', heap_bij_shared ps li0 @ sat γb⌉ -∗
  ⌈heap_bij_shared_auth (rhbv_combine inja' injb') @ sat γ⌉ -∗
  ...
  ∃ hm',
  (⌈heap_in_bij_inv (rhbv_core_a inja' injb') [] @ sat γa⌉ ∗
  ⌈heap_in_bij_inv (rhbv_core_b inja' injb') [] @ sat γb⌉) ∗
  (** interesting part ends here *)
  ...
  ⌈heap_bij_inv_i hm' @ sat γb⌉ ∗
  ⌈heap_bij_inv_s hb @ sat γ⌉ ∗
  ⌈heap_bij_shared_auth (rhbv_combine inja' injb') @ sat γ⌉ ∗ ⌜h_static_provs hm' = h_static_provs hm⌝.
Proof.
  iIntros "????????#Hsa#Hsb???".
  rewrite /heap_in_bij_inv.
  have Hempty : ∀ k, k ∈ [] ↔ False by set_solver.
  setoid_rewrite Hempty. setoid_rewrite bi.False_or.
  rewrite {2}rhbv_combine_to_b big_sepM_fmap.
  setoid_rewrite (weak_embed_big_sepM _ _ (rhbv_core_a _ _)).
  setoid_rewrite (weak_embed_big_sepM _ _ (rhbv_core_b _ _)).
  setoid_rewrite big_sepM_rhbv_core_a_b; try apply _.
  ...
  iIntros "!>" (???[?[??]]%rhbv_core_b_lookup_Some) "(?&?&Hta&Htb&?&Hinva&Hinvb&?&?&<-) [[%[%[??]]] [%[%[%Htot[Hi[Hs Hv]]]]]]".
  erewrite lookup_total_correct in Htot |- * => //. simplify_eq/=.
  iDestruct (rhbv_split_val_map with "Hsa Hsb [$] [$]") as (bm) "#[? ?]". simpl.
...
Lemma rec_heap_bij_vertical m hinit `{!VisNoAng m.(m_trans)} :
  trefines (rec_heap_bij hinit (rec_heap_bij hinit m))
           (rec_heap_bij hinit m).
Proof.
  unshelve apply: mod_prepost_combine_bi.
  set (Σ := #[satΣ heap_bijUR]). have ? : satG Σ heap_bijUR by apply _. clearbody Σ.
    ...
    exact (λ pl _ _ _,
      ∃ inja injb inj statics,
        if pl is Env then
          ...
          ⌜rhbv_combine inja injb ⊆ inj⌝ ∗ ∃ hm,
          ⌜h_static_provs hm = statics⌝ ∗
          ⌈[∗ map] ps↦li0∈inj, heap_bij_shared ps li0 @ sat γ⌉ ∗
          ⌈heap_bij_shared_auth inja @ sat γa⌉ ∗
          ⌈heap_bij_shared_auth injb @ sat γb⌉ ∗
          ⌈heap_in_bij_inv (inja ∖ rhbv_core_a inja injb) [] @ sat γa⌉ ∗
          ⌈heap_in_bij_inv (injb ∖ rhbv_core_b inja injb) [] @ sat γb⌉ ∗
          ([∗ map] ps↦li0 ∈ rhbv_core_b inja injb, rhbv_residual γa γb li0) ∗
          ...
          in this proof.) *)
          ⌈heap_bij_statics statics @ sat γa⌉ ∗
          ⌈heap_bij_statics statics @ sat γb⌉ ∗
          ⌈heap_bij_statics statics @ sat γ⌉
        else
          ⌜inj = rhbv_combine inja injb⌝ ∗
          ⌈[∗ map] ps↦li0∈inja, heap_bij_shared ps li0 @ sat γa⌉ ∗
          ⌈[∗ map] ps↦li0∈injb, heap_bij_shared ps li0 @ sat γb⌉ ∗
          ⌈heap_bij_shared_auth inj @ sat γ⌉ ∗
          heapUR_trader (sat γ) (sat γa) own_heap_i own_heap_i ∗
          heapUR_trader (sat γ) (sat γb) own_heap_s own_heap_s ∗
          ⌈heap_bij_statics statics @ sat γa⌉ ∗
          ⌈heap_bij_statics statics @ sat γb⌉ ∗
  ...
  - move => /= e ? h.
    ...
    iExists ∅. rewrite /heap_in_bij_inv !big_sepM_empty. iSplit!. {
  ...
  - move => /= ??????. iIntros "??? [[Hinv ?] _] (%inja & %injb & %inj & %statics & % & %  & % & ? & Hsha & Hshb & Hinja & Hs)". simplify_eq.
    ...
    iDestruct (heap_bij_statics_eq with "Hs1 Hs2") as %Heq1.
    iDestruct (heap_bij_statics_eq with "Hs Hs2") as %Heq2.
    iDestruct (heap_bij_shared_lookup_big with "Hsh [$]") as %?.

    iAssert ⌜inj' ##ₘ injb ∖ rhbv_core_b inja injb⌝%I as %?. {
      rewrite map_disjoint_spec.
      iIntros (?????).
      iDestruct (heap_in_bij_inv_borrow with "Hinj") as (???) "(?&?&Hp&?)"; [apply not_elem_of_nil|done|].
    ...
    set (inja' := (rhbv_proj_a inj' inja injb X)).
    set (injb' := (rhbv_proj_b inj' inja injb X)).

    have -> : inj' = rhbv_combine inja' injb' by
      apply rhbv_combine_proj; [by etrans |done|set_solver..].
    ...
    iApply (weak_embed_bupd_intro (sat γa) with "[$]").
    iMod (heap_bij_shared_alloc_big _ inja' with "Hsha") as "[? #Hshabig]".
    { apply rhbv_proj_a_subseteq. }
    iIntros "!> Hγa". iFrame "Hshabig".

    (** Update the residual *)
    iMod (rhbv_alloc_residual (rhbv_core_b inja' injb' ∖ rhbv_core_b inja injb) with "[$] [$] [$] [$]") as (hm') "[Hbs' [Hγa [Hγb[?[?%Heqm]]]]]". {
      move => ? /elem_of_map_img[? /lookup_fmap_Some[? [? ]]]. subst.
      move => /rhbv_core_b_proj_fresh[|/(fresh_map_lookup_Some _ _ _ _)]; set_solver.
    } {
      move => ??? /lookup_fmap_Some[[??][? /rhbv_core_b_proj_fresh[|??]]]. 1: set_solver.
      move => /lookup_fmap_Some[[??][? /rhbv_core_b_proj_fresh[|??]]]. 1: set_solver. simplify_eq/=.
      by apply: fresh_map_bij.
    } { move => ?? /rhbv_core_b_proj_fresh[|//]. set_solver. } {
      apply equiv_empty_L. move => ? /elem_of_filter[? /elem_of_map_img[? /lookup_fmap_Some[l [? ]]]]. subst.
      move => /rhbv_core_b_proj_fresh[//|/fresh_map_is_Block]. 1: set_solver. by destruct l.1.
    }
    iDestruct (big_sepM_union_2 with "Hbs Hbs'") as "Hbs".
    rewrite map_difference_union. 2: { apply rhbv_core_b_proj_subseteq. }

    (** Create the small [heap_in_bij_inv]. *)
    iMod (rhbv_shared_to_ab with "Hγa Hγb Hta Htb [$] [$] [$] [$] [$] [$] [$] [$] [$]") as (?) "([??] & ? & ? & ? & ? & ? & ? & ? & ? & ? & %Heqm')".

    iDestruct (heap_in_bij_inv_combine with "Hinja [$]") as "Hinja".
    iDestruct (heap_in_bij_inv_combine with "Hinjb [$]") as "Hinjb".
    erewrite rhbv_recombine_a => //. erewrite rhbv_recombine_b => //.
    2: set_solver.

    (** Split the val_in_bij for the arguments. *)
    iDestruct (rhbv_split_val_list with "Hshabig Hshbbig [$] [$]") as (vms) "#[Hv1 ?]".
    iDestruct (big_sepL2_length (PROP:=uPred heap_bijUR) with "Hv1") as %?.
  ...
  - move => /= ????????. iIntros "??? [[Hinva Hva] _] [[Hinvb Hvb] _] (%inja & %injb & %inj & %statics & % & ? & ? & Hsh & Hta & Htb & Hsa & Hsb & #Hs)".
    ...
    iDestruct (heap_bij_statics_eq with "Hsa1 Hsa2") as %Heq1.
    iDestruct (heap_bij_statics_eq with "Hsa Hsa1") as %?.
    iDestruct (heap_bij_statics_eq with "Hsb1 Hsb2") as %Heq2.
    iDestruct (heap_bij_statics_eq with "Hsb Hsb1") as %?.
    iFrame "Hsa Hsb Hs".
    iDestruct (heap_bij_shared_lookup_big with "Hsha [$]") as %?.
    iDestruct (heap_bij_shared_lookup_big with "Hshb [$]") as %?.

    (** Update the large injection *)
    iApply (weak_embed_bupd_intro (sat γ) with "[$]").
    iMod (heap_bij_shared_alloc_big _ (rhbv_combine inja' injb') with "Hsh") as "[? #Hshbig]".
    { subst. by apply rhbv_combine_subseteq. }
    iIntros "!> ?". iFrame "Hshbig".

    (** Split the small invariants *)
    iDestruct (heap_in_bij_inv_split (rhbv_core_a inja' injb') with "Hinja") as "[? Hinja]"; [apply rhbv_core_a_subseteq|].
    iDestruct (heap_in_bij_inv_split (rhbv_core_b inja' injb') with "Hinjb") as "[? Hinjb]"; [apply rhbv_core_b_subseteq|].
    iFrame "Hinja Hinjb".

    (** Combine the small invariants to get the large invariant. *)
    iApply (weak_embed_bupd_intro (sat γ) with "[$]").
    iMod (rhbv_shared_from_ab with "[$] [$] [$] [$] [$] [$] [$] [$] [$] [$] [$]") as "[[??][?[?[? [? [? [? [Hsha Hshb]]]]]]]]".

    (** Combine the val_in_bij for the arguments. *)
    iDestruct (rhbv_combine_val_list with "Hsha Hshb [$] [$] [$]") as "#?".
